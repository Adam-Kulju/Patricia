...
int mat(board_info *board){
  ...
  return total_mat < 3000 ? 10000 : mat;
}
...
int filter(const std::string input, const std::string &output){
  ...
  while (std::getline(fin, line)){
    ...
    float result = atof(line.substr(line.rfind(" | ") + 2).c_str());

    eval *= color == Colors::White ? 1 : -1;

    int material = mat(&board) * (color == Colors::White ? 1 : -1);

    if (abs(material) != 10000 && 
    ((eval > material + 400 && material < 100 && result == 1) || (eval < material - 400 && material > -100 && result == 0)))
    {
      filtered_lines++;
