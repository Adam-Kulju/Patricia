...
public:
  ...
  void add_add_sub_sub(int piece1, int from1, int to1, int piece2, int from2,
                       int to2);
  void pop();
  int evaluate(int color, const Position &position);
...
void NNUE_State::add_sub(int from_piece, int from, int to_piece, int to) {
  ...
  for (size_t i = 0; i < LAYER1_SIZE; ++i) {
    m_curr[1].white[i] = m_curr->white[i] +
                         content.feature_v[white_to * LAYER1_SIZE + i] -
                         content.feature_v[white_from * LAYER1_SIZE + i];

    m_curr[1].black[i] = m_curr->black[i] +
                         content.feature_v[black_to * LAYER1_SIZE + i] -
                         content.feature_v[black_from * LAYER1_SIZE + i];
  }
...
void NNUE_State::add_sub_sub(int from_piece, int from, int to_piece, int to,
  ...
  for (size_t i = 0; i < LAYER1_SIZE; ++i) {
    m_curr[1].white[i] = m_curr->white[i] +
                         content.feature_v[white_to * LAYER1_SIZE + i] -
                         content.feature_v[white_from * LAYER1_SIZE + i] -
                         content.feature_v[white_capt * LAYER1_SIZE + i];

    m_curr[1].black[i] = m_curr->black[i] +
                         content.feature_v[black_to * LAYER1_SIZE + i] -
                         content.feature_v[black_from * LAYER1_SIZE + i] -
                         content.feature_v[black_capt * LAYER1_SIZE + i];
  }
...
void NNUE_State::add_add_sub_sub(int piece1, int from1, int to1, int piece2,
  ...
  for (size_t i = 0; i < LAYER1_SIZE; ++i) {
    m_curr[1].white[i] = m_curr->white[i] +
                         content.feature_v[white_to1 * LAYER1_SIZE + i] -
                         content.feature_v[white_from1 * LAYER1_SIZE + i] +
                         content.feature_v[white_to2 * LAYER1_SIZE + i] -
                         content.feature_v[white_from2 * LAYER1_SIZE + i];

    m_curr[1].black[i] = m_curr->black[i] +
                         content.feature_v[black_to1 * LAYER1_SIZE + i] -
                         content.feature_v[black_from1 * LAYER1_SIZE + i] +
                         content.feature_v[black_to2 * LAYER1_SIZE + i] -
                         content.feature_v[black_from2 * LAYER1_SIZE + i];
  }
...
inline void NNUE_State::update_feature(int piece, int square) {
  ...
  if constexpr (Activate) {
    add_to_all(m_curr->white, m_curr->white, content.feature_v,
               white_idx * LAYER1_SIZE);
    add_to_all(m_curr->black, m_curr->black, content.feature_v,
               black_idx * LAYER1_SIZE);
  } else {
    subtract_from_all(m_curr->white, m_curr->white, content.feature_v,
                      white_idx * LAYER1_SIZE);
    subtract_from_all(m_curr->black, m_curr->black, content.feature_v,
                      black_idx * LAYER1_SIZE);
  }
}
...
void NNUE_State::reset_nnue(Position &position) {
  ...
  m_curr->init(content.feature_bias);
